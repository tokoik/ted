...
//	rectangle(frame, Point(left, top), Point(right, bottom), Scalar(0, 0, 255), -1);
	...
	Mat roi_image(frame, cv::Rect(left, top, right-left, bottom-top));
	Mat roi_image_resize = roi_image.clone();

	blur(roi_image, roi_image, cv::Size(50, 50));//コピーしない画像の処理


//	resize(roi_image, roi_image_resize, cv::Size(), 0.01, 0.01);//
//	resize(roi_image_resize, roi_image, cv::Size(), 100.0, 100.0);


//	rectangle(roi_image, Point((right-left)/2, (bottom-top)/2), Point(right-left, bottom-top), Scalar(0, 255, 255), -1);
//	rectangle(roi_image, Point(0,0), Point(right - left, bottom - top), Scalar(0, 255, 255), -1);

		//Draw a rectangle displaying the bounding box01
//	rectangle(frame, Point(left, top), Point(right, bottom), Scalar(0, 0, 255), -1);
	...
	for (int i = 0; i < feat_h; i++)
	{
		for (int j = 0; j < feat_w; j++)
		{
			...
			if (box_prob > this->confThreshold)
			{
				...
				float xmin = max((cx - pred_ltrb[0] - padw) * ratiow, 0.f);  ///?原回到原?
				...
				for (int k = 0; k < 5; k++)
				{
					float x = ((ptr_kp[(k * 3) * area + index] * 2 + j) * stride - padw) * ratiow;  ///?原回到原?
...
void YOLOv8_face::detect(Mat& srcimg)
{
	...
	vector<Mat> outs;
	///net.enableWinograd(false);  ////如果是opencv4.7，那就需要加上?一行
