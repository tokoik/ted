...
**手のトラッキング喪失時の非表示化設計**:
OpenXRで手が非アクティブまたは関節姿勢が無効になった場合と、Leap Motionの新しいフレーム内にそれまでの手が存在しなくなった場合には、該当する片手22個の行列へ**ゼロ行列（全要素 `0.0f`）**を書き込みます。これにより、その手を参照するモデルの変換が縮退して非表示になります。Leap Motionから新しいフレームが届いていないだけの場合は、更新レート差による点滅を避けるため直前の姿勢を維持します。
