...
```text
親ノードの変換
  × controller または remote_controller の姿勢
  × position
  × rotation
  × scale
```

したがって、`position`、`rotation`、`scale` はcontroller姿勢から見たローカル変換です。例えば頭部controllerの子を `[0, 0, -0.5]` に置くと、その物体は頭部の正面0.5mに固定され、頭部回転と一緒にオフセット方向も回転します。

同じ頭部姿勢を親子の両方へ指定すると二重適用になります。特にリモート側は、`remote_controller: 0` を外側ノードで一度だけ適用し、その子の手モデルではcontroller 2以降だけを使用します。
...
HMD、ゲームパッド、Leap Motion、OpenXR Hand Trackingから取得した位置姿勢は、共有メモリ上の変換行列テーブルに格納されます。JSONの `"controller"` または `"remote_controller"` でインデックスを指定すると、対応する位置姿勢に3Dモデルを追従させられます。

**手のトラッキング喪失時の非表示化設計**:
OpenXRで手全体が非アクティブになった場合と、Leap Motionの新しいフレーム内にそれまでの手が存在しなくなった場合には、該当する片手22個の行列へ**ゼロ行列（全要素 `0.0f`）**を書き込みます。これにより、その手を参照するモデルの変換が縮退して非表示になります。一時的に一部のOpenXR関節姿勢が無効になった場合や、Leap Motionから新しいフレームが届いていないだけの場合は、更新レート差による点滅を避けるため直前の姿勢を維持します。

0番と1番には、OpenXRの左右眼位置の中点を回転中心とする同一の頭部中心姿勢 `T_head * R` を格納します。描画時は眼ごとのview行列 `R^-1 * T_eye^-1` を使うため、頭部中心へ追従しながら左右眼の相対位置による両眼視差を維持できます。ヘッドトラッキングOFF時の0番と1番は恒等行列です。
...
この例では子ノードの位置が頭部中心を基準とするため、頭を動かしても照準の見かけの位置は変わりません。左右眼から描画した場合は、それぞれの眼と頭部中心の相対変換だけが残るため、頭部への固定と両眼視差を両立できます。

### 行列番号の対応表

| 番号 | 対応する部位 / データ源 |
| :--- | :--- |
| **0** | ローカルHMDの頭部中心姿勢 `T_head * R`（ヘッドトラッキングOFF時は恒等行列） |
| **1** | 0番と同じ頭部中心姿勢。将来の眼別参照との互換用スロット |
