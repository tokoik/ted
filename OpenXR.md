...
`XR_EXT_hand_tracking` は任意機能です。拡張とsystemの両方が対応する場合だけ左右のhand trackerを作成します。各フレームの予測表示時刻で関節を取得し、従来のLeap Motion用テーブルと互換な22姿勢へ変換して `Scene::setLocalHandAttitudes()` へ渡します。
