...
| `swap_camera_eyes` | 真偽値 | `true` の場合、入力後の左画像と右画像を入れ替えて各眼へ表示します。 |
...
フレーム内の物理的な配置と実際の左右眼が逆の場合は、配置を変えずに
`"swap_camera_eyes": true` を指定します。この設定はSBS/TABだけでなく、
左右別入力やリモート入力にも同じように適用されます。
