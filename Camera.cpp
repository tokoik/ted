...
void Camera::recv()
{
  ...
  {
    // 姿勢データを受信する
    const auto ret{ network.recvData(recvbuf, maxFrameSize) };
...
void Camera::send()
{
  ...
  while (run[camL])
  {
    ...
    // 左フレームの保存先 (変換行列の最後)
    auto data{ reinterpret_cast<uchar*>(body + head[camCount]) };
...
int Camera::startWorker(unsigned short port, const char* address)
{
  ...
  // 作業者として初期化する
  const auto ret{ network.initialize(2, port, address) };
  if (ret > 0) return ret;
